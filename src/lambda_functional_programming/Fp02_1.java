 ...
 */
    ...
    //1) Ardışık list elementlerini aynı satırda aralarında boşluk bırakarak yazdıran bir method oluşturun.(FunctionL ve metot refrans ile)
...
public static void tekElemanlarinKareleriniYazdir(List<Integer> list){
        ...
        list.stream().filter(Utils::tekElemanSec).map(Utils::karesiniAl).forEach(Utils::ayniSatirdaBosluklaYazdir);
}
...
public static void tekrarsizTekElemanlarinKupunuYazdir(List<Integer>list){
        ...
        list.stream().distinct().filter(Utils::tekElemanSec).map(Utils::kupunuAl).forEach(Utils::ayniSatirdaBosluklaYazdir);

        // map herbir elemanin degeri degisecegi icin kullanilir
}
